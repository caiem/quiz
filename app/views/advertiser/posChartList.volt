        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body table-responsive no-padding">
                ...
                <table class="table">
                    <tr><th>时间</th><th>广告位名称</th><th>请求数</th><th>响应数</th><th>曝光量</th><th>CPM曝光量</th><th>点击量</th><th>点击率(‰)</th><th>填充率(‰)</th><tr>
                    <tr class="fixed_head" style="display:none;background-color: aliceblue"><th>时间</th><th>广告位名称</th><th>请求数</th><th>响应数</th><th>曝光量</th><th>CPM曝光量</th><th>点击量</th><th>点击率(‰)</th><th>填充率(‰)</th><tr>
                    {% for item in tableData%}
                    <tr><td>{{item['date']|default('-')}}</td><td>{{item['pos_name']|default('-')}}{% if item['pos_name'] is not empty%} <i data-content='{{item['pos']|json_encode}}' style="cursor: pointer;" class="fa fa-question-circle posHelp"></i>{% endif %}</td><td>{{item['request']|default('-')}}</td><td>{{item['response']|default('-')}}</td><td>{{item['pv']|default('-')}}</td><td>{{item['cost_pv']|default('-')}}</td><td>{{item['cost_click']|default('-')}}</td><td>{{item['rate']|default('-')}}</td><td>{{item['fill_rate']|default('-')}}</td></tr>
                    {% endfor %}
                </table>
            </div>
            <!-- /.box-body -->
...
$(document).ready(function(){
  ...
  $(".download").click(function(){
    var form = $("#query_form");
    form.attr("action","/Statis/download");
    form.submit();
    form.attr("action","");
  });
