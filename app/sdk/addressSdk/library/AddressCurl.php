...
class AddressCurl {
    ...
    public function execute() {
        ...
        $this->curl_opt_arr = $this->options;
        $this->options();
...
//                $logger = new Log();
        $logger = new AddressSdkLog(); /*by wantao：调用方比如thinkphp那边调用的时候有个同名的Log类,导致重定义错误,所以对sdk的类名称加前缀AddressSdk,以避免以后类似问题,其他类命名会慢慢边做边修改 */
        // Request failed
        if (isset($this->info['http_code']) && 200 == $this->info['http_code']) {
        	curl_close($this->session);
            $this->last_response = $this->response;
            $this->set_defaults();
            if (ADDRESS_SDK_LOG_INFO_FLAG) {
                $logger->writeLog('sdk version:' . ADDR_APP_VERSION . ", cUrl url : " . $this->info['url']
                        . ', params : ' . json_encode($this->params)
                        . ', content_type : ' . $this->info['content_type']
                        . ', total_time : ' . $this->info['total_time']
                        . ', last_response : ' . $this->last_response, 'info'
                );
            }
            ...
            foreach ($this->curl_opt_arr as $k => $v) {
                $curl_header[array_search($k, $constants['curl'], TRUE)] = $v;
            }
            $logger->writeLog('sdk version:' . ADDR_APP_VERSION . ", $errno : $error"
                    . ', cUrl url : ' . $this->url
                    . ', params : ' . json_encode($this->params)
                    . ', cUrl info : ' . json_encode($this->info)
                    . ', cUrl header : ' . json_encode($curl_header)
                    . ', response : ' . $this->response
            );
            $this->error_code = $this->error_code ? $this->error_code : 10086;
            $result = array('status'=>$this->error_code, 'message'=>$this->error_string, 'data'=>false);
