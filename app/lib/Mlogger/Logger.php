...
class Logger{
    ...
    {
        $path              = isset($config['path']) ? $config['path'] : '/data';
        $filename          = isset($config['filename']) ? $config['filename'] : 'product.log';
        self::$destination = rtrim($path,'/').'/'.$filename;
        self::$source      = isset($config['source']) ? $config['source'] : '';
    }
     ...
     * @return bool
     */
    protected static function write($msg, $type = 'info', $force = false)
    {
        // 封装日志信息
